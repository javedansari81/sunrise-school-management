#!/usr/bin/env python3
"""
Complete API server with authentication and CRUD operations
Connects directly to PostgreSQL with full functionality
"""
...
app = FastAPI(
    ...
    description="Complete API with authentication and CRUD operations",
    version="1.0.0"
)
...
# Update FastAPI app to show both auth methods in Swagger
app = FastAPI(
    title="Sunrise School Management API",
    description="Complete API with authentication and CRUD operations",
    version="1.0.0"
...
@app.patch("/api/v1/leaves/{leave_id}/reject")
...
@app.patch("/api/v1/expenses/{expense_id}/reject")
...
async def get_dashboard_stats():
    ...
    try:
        ...
        recent_leaves = await conn.fetch("""
        ...
        recent_expenses = await conn.fetch("""
            SELECT e.id, e.title, e.total_amount, e.status, e.created_at
            FROM expenses e
        ...
        return {
            ...
            "recent_activities": {
                "leaves": [serialize_record(record) for record in recent_leaves],
                "expenses": [serialize_record(record) for record in recent_expenses]
            }
        }
    finally:
        await conn.close()
