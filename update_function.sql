-- Update the enable_monthly_tracking_complete function to fix the balance_amount issue
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            ...
            -- Get fee structure from existing record
            SELECT fs.* INTO fee_structure_record
            FROM fee_structures fs
            JOIN fee_records fr ON fs.id = fr.fee_structure_id
            WHERE fr.id = v_fee_record_id;
            
            -- Fallback: get fee structure by class if not found
            IF fee_structure_record IS NULL THEN
                SELECT fs.* INTO fee_structure_record
                FROM fee_structures fs
                WHERE fs.class_id = student_record.class_id 
        ...
        -- Calculate monthly amount (divide annual fee by 12)
